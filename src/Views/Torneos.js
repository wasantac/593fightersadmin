...
const Torneos = () => {
    ...
    let handleCrear = (e) => {
        ...
        axios
            ...
            .then((res) => {
                Swal.fire("Torneo Creado", `${titulo}`, "success").then(() => {
                    window.location.reload();
                });
            })
            .catch((err) => {
                Swal.fire("Error", `${err}`, "error").then(() => {
                    window.location.reload();
                });
            });
        e.preventDefault();
    ...
    let handleEdit = (e) => {
        ...
        axios
            .put(`/torneos?token=${localStorage.getItem("token")}`, {
                ...torneo,
                fecha: startDate,
                id: datos._id,
            })
    ...
    const [participantesDelete, setParticipantesDelete] = useState([]);
    const [idTorneo, setIdTorneo] = useState("");
    useEffect(() => {
        if (escoger) {
            setParticipantesDelete(JSON.parse(escoger).participantes);
            setIdTorneo(JSON.parse(escoger)._id);
        }
    }, [escoger]);
        ...
        <Container>
            ...
            <Collapse in={abrir.verTorneos} className="card shadow">
                <ListGroup>
                    {torneos.map((item, key) => {
                        return (
                            <TorneoItem key={key} torneo={item}></TorneoItem>
                        );
                    })}
                </ListGroup>
            </Collapse>
            <Selector
                value="Eliminar Participantes"
                className="mt-5"
                onClick={() => {
            ...
            <Collapse in={abrir.participantes} className="card shadow">
                <div>
                    ...
                    <ParticipanteLista
                        ...
                        id={idTorneo}
                        setParticipantes={setParticipantesDelete}
                    ></ParticipanteLista>
                </div>
            </Collapse>
        </Container>
